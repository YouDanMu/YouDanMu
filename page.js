...
/* Since YouTube's internal API is compressed, we cannot simply reuse the object path
since it changes very frequently. So this is a simple helper for us to analyze the
player object's structure and try to locate the `subscribe` function with unique
characteristics of its parents and siblings. */
function findSubscribe(player) {
    var props = Object.getOwnPropertyNames(player);
    for (var i in props) {
        var prop = player[props[i]];
        if (prop != undefined
            && typeof prop.subscribe === 'function'
            /* There exists two same structures of `subscribe`'s parent, the only
            way to differentiate them is by looking at a siblibg array's length. */
            && prop.o.length < 300) {
            /* We need to bind the function for outer client to call on. */
            return prop.subscribe.bind(prop);
        }
    }
    return false;
}
...
function hooker(player) {
    ...
    var subscribe = findSubscribe(player);
    if (!subscribe) {
        console.error("[YDM]: Cannot find subscribe function");
        return;
    }
    subscribe("onStateChange", function (state) {
